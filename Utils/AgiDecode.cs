...
{
    internal static class AgiDecoder
    {
        public static Bitmap? DecodeAgi(byte[] data)
        {
            if (data == null)
                return null;

            using var ms = new MemoryStream(data, false);
            return TryDecodeImage(ms);
        }

        public static bool DecodeAgiToPng(byte[] data, string outputPath)
        {
            var bmp = DecodeAgi(data);
            if (bmp == null)
                return false;
        ...
        static Bitmap? TryDecodeImage(Stream stream)
        {
        ...
        static Bitmap? Read4bpp(Stream s, int width, int height, AgiEntry tex, AgiEntry clut)
        {
        ...
        static Bitmap? Read8bpp(Stream s, int width, int height, AgiEntry tex, AgiEntry clut)
        {
            uint palOff = clut.DataOffset;
            if (palOff == 0) return null;
        ...
        static Bitmap? Read16bpp(Stream s, int width, int height, AgiEntry tex)
        {
            int pixelDataOffset = (int)tex.DataOffset;
        ...
        static Bitmap? Read24bpp(Stream s, int width, int height, AgiEntry tex)
        {
            int pixelDataOffset = (int)tex.DataOffset;
